<%@page import="dto.반품Dto"%>
<%@page import="java.util.ArrayList"%>
<%@page import="dao.반품Dao"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    ArrayList<반품Dto> listreturn = null;
    
    String loginId = (String)session.getAttribute("id");
    if("반품완료".equals(returnstatus)) {
    	listreturn = dao.get반품상태list(loginId, "반품완료");
    } else if("반품요청".equals(returnstatus)) {
    	listreturn = dao.get반품상태list(loginId, "반품요청");
    } else if ("수거중".equals(returnstatus)) {
    	listreturn = dao.get반품상태list(loginId, "수거중");
    } else if("수거완료".equals(returnstatus)) {
    	listreturn = dao.get반품상태list(loginId, "수거완료");
    } else{
    	listreturn = dao.get반품관리list(menu, date, date2, choose, name, res, n);
    }
    %>
<!DOCTYPE html>
<html>
...
<link rel="stylesheet" href="../css/ReturnManage.css" />
<link rel="stylesheet"
...
<script>
	...
	$(document).ready(function() {
		$("#buttonst1").click(function() {
			let button_text = $(this).find('span.buttonsize1');
	...
	$(function() {
		...
		$("#return_request").click(function(){
			let er = $("#return_request_1").text();
			location.href = "../project3_sv/ReturnManage.jsp?returnstatus=반품요청"; 
		});
		$("#return_comp").click(function(){
			let er = $("#return_comp_1").text();
			location.href = "../project3_sv/ReturnManage.jsp?returnstatus=반품완료"; 
		});
		$("#return_collection_comp").click(function(){
			let er = $("#return_collection_comp_1").text();
			location.href = "../project3_sv/ReturnManage.jsp?returnstatus=수거완료"; 
		});
		$("#return_collection").click(function(){
			let er = $("#return_collection_1").text();
			location.href = "../project3_sv/ReturnManage.jsp?returnstatus=수거중"; 
		});
		$(document).ready(function() {
		...
		$("#searchbutton").click(function(){ /*데이터를 넘기는 쪽  */
			...
			let res = $("#sel_date2").val();
			
			if (choose === '전체' && res === '전체') {
			    alert("반품처리상태 또는 상세 검색조건을 입력하신 후 검색 가능합니다.");
			    return; 
			  }
			...
			let encodedRes = encodeURIComponent(res);
			
			location.href = "../project3_sv/ReturnManage.jsp?menu=" + encodedMenu + "&date=" + encodedDate +
		...
		$(".searchbuttonsection, .searchbutton3, .searchbutton4, .size_medium, #searchbutton3, .prepare, .exeldown2 ,.buttonsizetype").click(function() {
					alert("준비중입니다");
				});
	...
	window.onload = function() {
		...
		let instance = new Grid({
			...
			data : [ 
				...
				int cnt3 = 0;
				int cnt4 = 0;
				for(반품Dto dto : listreturn){
					
						if(dto.getRss().equals("반품요청")){
							cnt++;
						}
				...
				returnstatus : '<%=dto.getRss()%>',
				returncollectionstatus : '<%=dto.getCs()%>',
				paydate : '<%=dto.getPd()%>',
				returnrequestdate : '<%=dto.getRrd()%>',
				returnreason : '<%=dto.getRr()%>',
				returngrant : '<%=dto.getRad()%>',
				returnend : '<%=dto.getRcd()%>',
	...
	<div id="right_side" class="fl">
		<div class="backgroundtheme">
			<div class="width12 fl">
				...
				<div class="smart1">
					<strong class="strongfont fl">구매자가 요청한 반품 주문건에 대해 완료/거부처리를
						진행하실 수 있습니다.</strong>
					<button id="buttonst1"
					...
					<div id="exchange" class="listlist list_font"
						style="display: none;">
						<li>판매자께서 직접 반품접수 하고자 하신다면 배송현황 관리 메뉴에서 처리해주세요.</li>
						<li>반품요청 사유가 구매자 귀책인 경우 반품배송비가 자동 청구되고 판매자 귀책인 경우 청구되지 않습니다.</li>
						<li>수거완료 후 +3영업일 이내 반품처리를 진행하지 않으시면 페널티가 부과되오니 주의해 주세요.</li>
					</div>
					<div style="clear: both;"></div>
				</div>

				<div class="rmsection">
					<div class="rmsection2">
						<strong class="rmfont">주문유형을 선택해주세요!</strong>
						<div class="rmsection2_1 rmfont1">
							<div class="rmsection2_0">
								<div class="rmsection2_2">
								...
								<div class="rmsection2_2">
									<label class="seller-list label_sty"> 
									<input id="common_order" type="checkbox" name="productStatus">
										일반주문<span></span>
									</label>
								</div>
								<div class="rmsection2_2">
									<label class="seller-list label_sty"> 
										<input id="today_start" type="checkbox" name="productStatus">
											오늘출발<span></span>
									</label>
								</div>
								<div class="rmsection2_2">
									<label class="seller-list label_sty"> 
										<input id="reservation" type="checkbox" name="productStatus">
											예약구매<span></span>
									</label>
								</div>
								<div class="rmsection2_2">
									<label class="seller-list label_sty"> 
										<input id="subscribe" type="checkbox" name="productStatus">
											정기구독<span></span>
									</label>
								</div>
								<div class="rmsection2_2">
									<label class="seller-list label_sty seller_list_"> 
										<input id="rental" type="checkbox" name="productStatus" class="seller_list__"> 
										렌탈<span></span>
									</label>
								</div>
								...
								<div class="rmsection2_2 rmsection2_21">
									<label class="seller-list label_sty"> 
										<input type="checkbox" name="productStatus"> 
										풀필먼트 배송건 제외하고 보기<span></span>
				...
				<div class="rmsectionstyle">
					<div class="seller-status form-section">
						<div class="delivery_return returniconstyle3">
							...
							<ul class="seller-status-list mstyle">
								...
								<li>
									<div class="iconstyle">
										<span class="status-icon"> 
											<i class="icon-close returniconstyle1 returniconstyle2">
												<img src="data:image/png;base64,">
											</i>
										</span>
										<div class="text">
											<p>자동 환불대기</p>
								...
								<li>
									<div class="iconstyle">
										<span class="status-icon"> 
											<i class="icon-close returniconstyle1 returniconstyle2">
												<img src="https://sell.smartstore.naver.com/o/v3/temp/static/npay/app/img/ico_auto_collect_request_fail-aee7888da20c2fecc6849d94a1de9250.png" class="returniconstyle7 truckstyle">
											</i>
										</span>
										<div class="text">
											<p>자동 수거요청 실패</p>
						...
						<div class="delivery_return returniconstyle4 ">
							<strong class="greenstyle redstyle2">반품진행중/완료 주문건을<br>확인해주세요.</strong>
							<ul class="seller-status-list">
								<li>
									<div class="iconstyle <%=("반품요청".equals(request.getParameter("returnstatus")))?"on":""%>">
										<span id="return_request" class="status-icon <%=("반품요청".equals(request.getParameter("returnstatus")))?"on":""%>"> 
											<i class="icon-suspension returniconstyle1 returniconstyle2">
												<img src="data:image/png;base64,">
											</i>
										</span>
										<div class="text">
											<p>반품요청</p>
											<a href="#" class="_3a4NLUdd2p">
												<strong><%=cnt%></strong> <em>건</em>
											</a>
										</div>
									</div>
								</li>
								<li>
									<div class="iconstyle <%=("수거중".equals(request.getParameter("returnstatus")))?"on":""%>">
										<span id="return_collection" class="status-icon <%=("수거중".equals(request.getParameter("returnstatus")))?"on":""%>"> 
											<i class="icon-close returniconstyle1 returniconstyle2">
												<img src="data:image/png;base64,">
											</i>
										</span>
										<div class="text">
											<p>반품수거중</p>
											<a href="#" class="_3a4NLUdd2p">
												<strong><%=cnt2%></strong> <em>건</em>
											</a>
										</div>
									</div>
								</li>
								<li>
									<div class="iconstyle <%=("수거완료".equals(request.getParameter("returnstatus")))?"on":""%>">
										<span id="return_collection_comp" class="status-icon <%=("수거완료".equals(request.getParameter("returnstatus")))?"on":""%>"> 
											<i class="icon-close returniconstyle1 returniconstyle2">
												<img src="data:image/png;base64,">
											</i>
										</span>
										<div class="text">
											<p>반품수거완료</p>
											<a href="#" class="_3a4NLUdd2p">
												<strong><%=cnt3%></strong> <em>건</em>
											</a>
										</div>
									</div>
								</li>
								<li>
									<div class="iconstyle <%=("반품완료".equals(request.getParameter("returnstatus")))?"on":""%>">
										<span id="return_comp" class="status-icon <%=("반품완료".equals(request.getParameter("returnstatus")))?"on":""%>"> 
											<i class="icon-close returniconstyle1 returniconstyle2">
												<img src="data:image/png;base64,">
											</i>
										</span>
										<div class="text">
											<p>반품완료</p>
											<a href="#" class="_3a4NLUdd2p">
												<strong><%=cnt4%></strong> <em>건</em>
											</a>
										</div>
									</div>
								</li>
				...
				<div class="jy">
					...
					<div class="fl">
						<div id="ps" class="orderselectbox div_date" style="display: none;">
							<label id="lbl_date2" class="returniconstyle6 lbl_date_">전체</label> 
							<select id="sel_date2" class="selectoption lbl_date_">
								<option selected>전체</option>
								<option id="return_request_1">반품요청</option>
								<option id="return_collection_1">수거중</option>
								<option id="return_collection_comp_1">수거완료</option>
								<option id="return_comp_1">반품완료</option>
							</select>
						</div>
						<div class="fl">
							<div class="buttonsize2">
								...
								<button id="buttonst2" class="buttoncolor buttonsize folding rotate fl" onclick="spread('ps')">
						...
						<div class="jy2 gapstyle">
							<div>
								...
								<div class="manybut">
									<button class="exeldown2">
										<span class="searchitemset">반품처리 한번에 하기</span>
									</button>
									<button class="exeldown2">
										<span class="searchitemset">반품 완료처리</span>
									</button>
									<button class="exeldown2">
										<span class="searchitemset">반품 거부(철회)처리</span>
									</button>
									<button class="exeldown2">
										<span class="searchitemset">교환으로 변경</span>
									</button>
							...
							<div class="storeboard">
								<table class="labelline">
									...
									<tbody>
										<tr>
											<th class="storboardrow storeboardfont">반품처리
												<div class="tablerow2">
													<i class="tablerow3"> </i>
												</div>
											</th>
										...
										<tr>
											<th class="storboardrow storeboardfont">환불
												보류
												<div class="tablerow2">
													<i class="tablerow3"> </i>
												</div>
					...
					<footer class="storefooter">
						<div class="innerfooter">
							<ul class="provis bottomstyle">
								...
								<li class="li2">
									<a class="footerstyle" href="javascript:void(0)">책임의 한계와 법적고지</a>
								</li>
								<li class="li2">
									<a class="footerstyle" href="https://ips.smartstore.naver.com/main/rules/safety" target="_blank">안전거래가이드</a>
								</li>
								<li class="li2">
							...
							<ul class="provis bottomstyle">
								...
								<li class="liststyles">
									<a id="businessinfo" href="http://www.ftc.go.kr/bizCommPop.do?wrkr_no=2208162517" class="btn-box" target="_blank">사업자정보확인</a>
								</li>
							</ul>
							...
							<address class="naverlinkstyle2 bottomstyle">
								<p>
									<a class="navericon logo" href="https://www.navercorp.com/" target="_blank"></a>
								</p>
								<p class="naverlinkstyle3">Copyright ©</p>
								<a class="naverlinkstyle4 copy-navercorp" href="https://www.navercorp.com/" target="_blank">
