...
<% 		
	...
	ArrayList<CustomerInquiryDto> listCustomerInquiryDto = customerInquiryDao.select2(seller_id,inquiry_date,startNum,endNum);
	if(seller_id==null){
		seller_id="jiyoonjung";
		listCustomerInquiryDto = customerInquiryDao.select2(seller_id, inquiry_date,startNum,endNum);
		count = customerInquiryDao.count2(seller_id);
	}else {
		count = customerInquiryDao.count2(seller_id);
		...
		<div>
			...
			<div id="side_content4" class="dropcontent" style="display: none;">
				<div class="droptext">
					...
					<div>
						<a href="CustomerInquiry.jsp">고객문의
							관리</a>
					</div>
					<div>
...
<!-- 					<a href="" role="button" ng-click="vm.setPage(page)"> -->
					<div class="numbutton numbuttonstyle">
						...
						%>
							<a href="InquiryManage.jsp?seller_id=<%=seller_id%>&page=<%=i%>"><strong class="numbuttonstrong1"><%=i %></strong></a>
						<%	
						}
						%>
					</div>
