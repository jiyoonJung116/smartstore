...
<%@page import="dao.*"%>
	...
	CommonDto dto = null;
  	...
  	long serialNum = Long.parseLong(request.getParameter("serialNum"));
  	AnnouncementDao andao = new AnnouncementDao();
	AnnouncementDto andto = andao.select(serialNum);
	ArrayList<AnnouncementDto> andtoar = andao.select(seller_id);
  	int cnt = 0;
  	for(AnnouncementDto d : andtoar) {
  		cnt++;
  	}
...
<head>
	<meta charset="UTF-8">
	<title>DailyLeftAM_content</title>
	<link rel="stylesheet" href="../css/DailyLeftAM_content.css" />
...
<div class="pdcontent">
			<div>
				<div class="pd_title">
					<strong class="pd_titles">전체
						<span class="pd_titlespan"><%=cnt %></span>
					</strong>
					<div class="fr">
						<div class="myanswer">
							<a href="DailyLeftANCM.jsp" class="totalanmlook">전체 공지사항 보기</a>
						</div>
						<div style="clear:both;"></div>
					</div>
				</div>
				<div>
					<div class="pd_inq_con">
						<div class="pd_inq_contt">
							<span class="fontcolor">[상품]</span>
							<span><%=andto.getSubject() %></span>
						</div>
						<div class="pd_inq_contt2">
							<span><%=andto.getRegDate() %></span>
							<span style="margin-left : 10px;">상품번호 <%=andto.getPnumber() %></span>
						</div>
					</div>
					<div class="pd_inq_content">
						<div><%=andto.getContent() %></div>					
					</div>
				</div>
