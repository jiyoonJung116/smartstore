...
<%@page import="dao.OrderConfirmDao"%>
...
<%
	...
	OrderConfirmDao dao = new OrderConfirmDao();
    ...
    String desasc= request.getParameter("desasc");
    
	
	ArrayList<주문확인Dto> listorder = dao.listOrderConfirm(menu, date, date2, choose, name, ors, n, desasc);
...
<script>
	...
	$(function() {
		...
		$("#searchbutton").click(function(){ /*데이터를 넘기는 쪽  */
			...
			let encodedOrs = encodeURIComponent(ors);
			
			location.href = "../project3_sv/OrderConfirm.jsp?menu=" + encodedMenu + "&date=" + encodedDate +
             "&date2=" + encodedDate2 + "&choose=" + encodedChoose + "&name=" + encodedName + "&ors=" + encodedOrs;
		});
	...
	window.onload = function() {
		...
		var instance = new Grid({
			...
			data : [ 
				<%
					for(주문확인Dto dto : listorder){
				%>
				{
				productordernum : '<%=dto.getPon()%>',
				ordernum : '<%=dto.getDen()%>',
				deliveryway_purchase : '<%=dto.getDw()%>',
				deliverycompany : '<%=dto.getDc()%>',
				invoicenum : '<%=dto.getIn()%>',
				buyer : '<%=dto.getNa()%>',
				buyerid : '<%=dto.getBi()%>',
				recipient : '<%=dto.getRn()%>',
				orderstatus : '<%=dto.getOs()%>',
				paydate : '<%=dto.getPayd()%>',
				productnum : '<%=dto.getPnumber()%>',
				pname : '<%=dto.getPname()%>',
				optioninfo : '0',
				optioncode : '0',
				quantity : '1',
				...
				productprice : '10',
				totalprice : '10',
				orderconfirmdate : '2023.04.18 14:18:50',
				deliverydeadline : '2023.04.25 14:18:50',
				shipmentprocessing : '2023-04-25',
				deliveryfeetype : '무료',
				shippingfeetype : '무료',
				totalshippingcost : '0',
				pcode : '1',
				receiptphone : '010-1234-5678',
				defaultdelivery : '서울특별시 마포구',
				detaildelivery : '신촌동 325',
				buyerphone : '010-1234-5678',
				postnum : '02654',
				postmessage : '<%=dto.getDm()%>',
				shippingplace : '서울특별시 마포구',
				orderdate : '<%=dto.getOdt()%>'
			},
			<%}%>
			]
		});
	...
	<div id="header">
		<div class="left-header fl">
			...
			<div class="fl topmenu">
				<span>쇼핑파트너센터</span> <span>커머스솔루션마켓</span> <span>네이버광고 
	...
	<div id="left_side" class="fl">
		...
		<div>
			...
			<div class="menu" onclick="sidebarbtn('side_content6', 'dropicon6')">
				<div>스토어 전시관리 <span class="upicon">N</span>
				</div>
	...
	<div id="right_side" class="fl">
		<div class="backgroundtheme">
			<div class="width12 fl">
				<div class="salehead">
					<h2 class="manage">발주(주문)확인/발송관리</h2>
					...
					<label id="lbl_namenum">수취인명</label> 
						<select id="sel_namenum" class="selectoption">
					...
					<input type="text" id="orderSearchInput" placeholder="상세조회 내용">
				...
				<div style="clear: both;"></div>
				<div class="smart1">
					<strong class="strongfont fl">발주확인(신규 주문건 확인) 및 발송처리, 발송지연 처리를 하실 수 있습니다.</strong>
					<button id="buttonst1" class="buttoncolor buttonsize folding rotate fl" onclick="spread('exchange')">
						<span class="buttonsize1">메뉴설명 펼치기</span>
					</button>
					<div style="clear: both;"></div>
					<div id="exchange" class="listlist list_font" style="display: none;">
						<li>발송기한 내 '발송처리'를 진행해 주세요. 기한 내 미 발송 시 페널티가 부과 됩니다. (발송기한은
							일반상품 기준 결제완료 후 3일, 단 상품마다 상이)</li>
						<li>'발송지연 처리'로 발송기한을 1회 연장할 수 있으며, 발송기한 전에 진행해 주세요.</li>
						<li>선택 된 배송방법에 따라 자동구매확정일이 달라질 수 있으며, 구매자가 문제 제기 시 자동구매확정이
							연장/불가 될 수 있습니다.</li>
						<li>정확한 송장번호와 구매자에게 고지·협의된 배송 방법을 사용해 주세요. 허위 송장번호 사용 등
							부정·불법행위는 엄격히 금지되며, 이에 따른 민·형사상의 책임은 판매자에게 있습니다.</li>
					</div>
					<div style="clear: both;"></div>
				</div>
				<div class="order_confirm">
					...
					<div class="fl">
						<div id="ps" class="orderselectbox div_date" style="display:none;">
							<label id="lbl_date2" class="ociconstyle6 lbl_date_">전체</label> 
						...
						<div class="order_confirm2 gapstyle">
							<div>
								<div class="totallistsize">
									...
									<div class="div_date">
										<label id="lbl_order">상품주문번호 내림차순</label> 
										<select id="sel_order" class="selectoption proudctorder">
											<option selected>상품주문번호 내림차순</option>
											<option>상품주문번호 오름차순</option>
										</select>
										<div style="clear: both;"></div>
									</div>
								...
								<div class="manybut">
									<button class="exeldown2">
										<span class="searchitemset">발주확인</span>
									</button>
									<button class="exeldown2">
										<span class="searchitemset">발송처리</span>
									</button>
									<button class="exeldown2">
										<span class="searchitemset">엑셀 일괄 발송처리</span>
									</button>
									<button class="exeldown2">
										<span class="searchitemset">발송지연 처리</span>
									</button>
									<button class="exeldown2">
										<span class="searchitemset">판매자 직접취소 처리</span>
									</button>
							...
							<div class="storeboard">
								<table class="labelline">
									...
									<tbody>
										<tr>
											<th class="storboardrow storeboardfont">주문확인 처리
												<div class="tablerow2">
													<i class="tablerow3"> </i>
												</div>
											</th>
										...
										<tr>
											<th class="storboardrow storeboardfont">발송처리 보류
												<div class="tablerow2">
													<i class="tablerow3"> </i>
												</div>
											</th>
											<td colspan="1">
												...
												<button type="button" class="buttonsizetype">
													<span>엑셀 일괄 발송처리</span>
												</button>
												<button type="button" class="buttonsizetype">
													<span>합포장 일괄 발송처리</span>
												</button>
												<button type="button" class="buttonsizetype">
													<span>송장수정</span>
												</button>
					...
					<footer class="storefooter" role="contentinfo">
						<div class="innerfooter">
							<ul class="provis bottomstyle">
								...
								<li class="li2">
									<a class="footerstyle" href="https://new-m.pay.naver.com/member/terms-policy/finance" target="_blank">전자금융거래이용약관</a>
								</li>
								<li class="li2">
							...
							<ul class="provis totalinfo bottomstyle">
								...
								<li>
									<a class="talktalkline totalinfo3" class="btn-box" target="_blank" href="https://talk.naver.com/ct/wcbgqj?frm=cstpn"><i></i> 
