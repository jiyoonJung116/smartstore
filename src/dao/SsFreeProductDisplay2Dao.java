...
public class SsFreeProductDisplay2Dao {
	public ArrayList<자유상품상세검색Dto> list상세검색(String search, long pNumber, String pName, String model, String scode, String choose, String order) {
		if(search==null)
			search = "전체";
		if(choose==null)
			choose = "전체";
		...
		if(pNumber==0)
			pNumber = 0L;
System.out.println("--- list상세검색 메서드, choose: " + choose);
		...
		String totalWhere = " WHERE p.category_id = c.category_id";
		...
		if (choose.equals("무료배송")) {
			if (search.equals("상품번호") || search.equals("상품명") || search.equals("모델명") || search.equals("판매자상품코드")) {
				switch (order) {
				case "인기도순":
					sql += sql2 + popular + free;
					break;
				case "누적판매순":
					sql += sql2 + where + free;
					break;
				case "최신등록순":
					sql += sql2 + regidate + free;
					break;
				case "리뷰수순":
					sql += sql2 + regidate + free;
					break;
		...
		} else if(choose.equals("전체") && search.equals("전체") && order.equals("전체") && pNumber==0) {
			sql+=totalWhere;
...
System.out.println("BEFORE return, listDetail.size() : " + listDetail.size());
		return listDetail;
	}
}
